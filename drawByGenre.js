...
function setGenre() {
  ...
  genres.forEach(genre => {
    ...
    // 장르 div를 클릭하면 selectGenre 배열에 담기
    t.addEventListener('click', () => {
      if (selectGenre.length == 0) {  // 배열에 아무것도 담겨있지 않으면
        selectGenre.push(genre.id); // 조건없이 push
      } else {  // 배열에 뭔가 담겨있긴 해
...
function highlightSelection() {
  ...
  clearBtn();
  if (selectGenre.length != 0) {  // 빈 배열이 아니라면
    selectGenre.forEach(id => { // 배열에 저장된 id값을 다시 받아 하이라이트class 추가
